...
describe("crawler tests", () => {
  test(
    "The crawler can return a list of match summaries (which contain match IDs) when " +
      "provided an HTML document containing a given player's match information.",
    async () => {
  ...
  test(
    "The crawler can return a user-readable & database-ready list of " +
      "match summaries and character details when provided an HTML document and JSON data",
    async () => {
      const testHtml = fs.readFileSync(
        path.resolve(__dirname, "test.html"),
        "utf-8"
      ...
      if (firstMatchDetails && firstMatchDetails.character_details.length > 0) {
        expect(firstMatchDetails.character_details[0]).toHaveProperty(
          "charname"
        );
