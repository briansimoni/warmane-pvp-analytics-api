...
Need to create a .env file and put the tablename that you're using in there. It should be in json format like this:
...
```bash
warmane-pvp-analytics$ sam build
```
...
```bash
warmane-pvp-analytics$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
warmane-pvp-analytics$ sam local start-api
warmane-pvp-analytics$ curl http://localhost:3000/
```
...
```bash
warmane-pvp-analytics$ sam logs -n WarmaneApiFunction --stack-name warmane-analytics-api-prod --tail
```
...
```bash
warmane-pvp-analytics$ pip install pytest pytest-mock --user
warmane-pvp-analytics$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name warmane-pvp-analytics
```
